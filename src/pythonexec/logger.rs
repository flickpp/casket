...
#[pyfunction(tag = "None")]
pub fn warn(py: Python, msg: &str, mut tags: Option<&PyDict>) -> PyResult<()> {
    let dict = tags.take().unwrap_or_else(|| PyDict::new(py));
    log(Level::Warn, msg, dict)
}

#[pyfunction(tags = "None")]
pub fn error(py: Python, msg: &str, mut tags: Option<&PyDict>) -> PyResult<()> {
    let dict = tags.take().unwrap_or_else(|| PyDict::new(py));
    log(Level::Error, msg, dict)
}
